...
- [x] Выбран STM32G0B1RET6 как центральный контроллер.
- [x] Принята загрузка patch/config PD-контроллеров из Flash MCU без отдельных SPI flash.
- [x] Разделены AON, PD1 и SYS I2C-шины.
- [x] Выбрана базовая архитектура питания: LMR51606, TPS55288 и AP2553.
...
- [x] Добавлены MCU, TPS26750, TPS65988, четыре TPS55288, LMR51606 и основные силовые цепи.
...
- [~] Блок входного TPS26750 собран, но требует исправлений выводов и полной проверки startup/power path.
- [~] Двухпортовый TPS65988 и два laptop power-converter канала собраны частично.
- [~] MCU имеет питание, reset, BOOT0, USB DFU и три I2C-шины, но GPIO map не завершён.
...
- [ ] Ограничить входную policy так, чтобы TPS55288 не получал больше 36 V; 48 V EPR запретить.
- [ ] Проверить hardware OVP/eFuse между входом и downstream converters.
- [ ] Рассчитать каждый TPS55288: токи ключей/дросселя, shunt, compensation, ILIM, MOSFET losses и thermal margin.
- [ ] Проверить DC-bias всех MLCC, особенно 20 V VBUS rails.
- [ ] Рассчитать бюджет `+3V3_MCU` и `+3V3_SYS`; подтвердить запас LMR51606/AP2553.
...
- [ ] Реализовать TPS55288 configuration и safe output sequencing.
